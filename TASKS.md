# TASKS.md — Agent Work Queue

Update this file before starting any task and after completing one.
One agent per file at a time. If your target file is already In Progress, stop and report.

## 🔴 In Progress


## 🟡 Queued
<!-- Format:
- [ ] Agent: [name] | Files: [paths] | Task: [what] | Priority: high/med/low
-->

## 🟢 Completed (keep last 10)
- [x] Agent: OpenCode | Files: backend/ | Task: Stage 6 catalogue — published reads, admin version/publish, illustrative seeds (36 tests green) | Commit: STAGE6
- [x] Agent: OpenCode | Files: backend/ | Task: Stage 5 conversations — persistence, ask, regenerate, import, owner-404 (31 tests green) | Commit: STAGE5
- [x] Agent: OpenCode | Files: backend/, src/app/api/v1/ | Task: Stage 4 auth — Argon2id sessions, cookies, CSRF, lockout, RBAC, admin gate, v1 proxy (24 tests green) | Commit: STAGE4
- [x] Agent: OpenCode | Files: backend/ | Task: Stage 3 RAG skeleton — lexical retrieval, extractive compose, citation contract, ingestion MVP (dense vectors + LLM generator pending OD-007/008) | Commit: STAGE3
- [x] Agent: OpenCode | Files: backend/ | Task: Stage 2 /search compatibility — frozen contract, stub RAG, rate limits, API-LEG-001..006 (9 tests green), proxy conformance via :3000 | Commit: STAGE2
- [x] Agent: OpenCode | Files: backend/ | Task: Stage 1 backend foundation — FastAPI skeleton, health/readiness, request-id, error envelope, tests (3 passed), compose defined-not-run | Commit: STAGE1
- [x] Agent: OpenCode | Files: src/, public/, README.md, AGENTS.md | Task: Convert OpenInsight medical app to BIS Chat (BIS/ISO/IEC sources, standards copy, hexagon-seal logo) | Commit: 0fbe8b1
- [x] Agent: Antigravity | Files: src/ | Task: Convert new Vite code from main branch to Next.js framework | Commit: 6fe6995
<!-- Format:
- [x] Agent: [name] | Files: [paths] | Task: [what] | Commit: [hash]
-->

## 🔒 Blocked
<!-- Format:
- [ ] Task: [what] | Blocked by: [reason or dependency]
-->
